...
func (ipc *IPCache) InjectLabels(ctx context.Context, modifiedCIDRs []string) (remainingCIDRs []string, err error) {
	...
	for i, prefix := range modifiedCIDRs {
		...
		if entryExists {
			...
			// previous iteration of this code hit one of the
			// 'injectLabels' cases below, thereby allocating an
			// identity. If we delete or update the identity for
			// 'prefix' in this iteration of the loop, then we must
			// balance the allocation from the prior iteration by
			// releasing the previous reference again.
		...
		if prefixInfo == nil {
			...
			if err != nil {
				...
				// TODO: Should we attempt to continue so that
				// other identities could be released?
